...
def astar_search(graph, start, goal):
    ...
    if start == goal:
        return path, explored_nodes
    path.append(start)
    path_cost = my_heuristic(start, goal)
    ...
    while len(frontier) > 0:
        ...
        path_cost_till_now, path_till_now = pop_frontier(frontier)
        current_node = path_till_now[-1]
        path_cost_till_now = path_cost_till_now - my_heuristic(current_node, goal)
        explored_nodes.append(current_node)
        # test goal condition
        if current_node == goal:
            return path_till_now, explored_nodes
        ...
        for neighbour in neighbours_list_str:
            ...
            extra_cost = 1
            neighbour_cost = extra_cost + path_cost_till_now + my_heuristic(neighbour, goal)
...
def greedy_search(graph, start, goal):
    ...
    while len(frontier) > 0:
        ...
        for neighbour in neighbours_list_str:
            ...
            #Greedy
            neighbour_cost =  my_heuristic(neighbour, goal)
...
# Adding a variable to store the heuristic used for the search.
my_heuristic = get_geographical_heutristic
if __name__ == '__main__':
    ...
    print("============ Greedy Search ================")
    path_greedy, explored_greedy = greedy_search(graph_neighbours, '0', '50')
    print("Path_astar:", path_greedy)
